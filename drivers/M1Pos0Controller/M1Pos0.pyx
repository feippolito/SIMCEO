cdef extern from "M1Pos0.h":
	ctypedef double real_T
	ctypedef struct ExtU_M1Pos0_T:
		real_T Feedback[42]   #              /* '<Root>/Feedback' */
	ctypedef struct ExtY_M1Pos0_T:
		real_T Output[42]     #              /* '<Root>/Output' */
	ExtU_M1Pos0_T M1Pos0_U
	ExtY_M1Pos0_T M1Pos0_Y
	void M1Pos0_initialize()
	void M1Pos0_step()
	void M1Pos0_terminate()
...
class M1Pos0: 
	'''
	This class was automatically generated from the simulink subsystem 'M1Pos0'.
	...
	def update(self, np.ndarray u):
		...
		for k in range(42):
			M1Pos0_U.Feedback[k] = __u[k]
		M1Pos0_step()

	def output(self):
		cdef int k
		for k in range(42):
			self.__yout[k] = M1Pos0_Y.Output[k]
