...
class mainFEMDriver:
  ...
  def driver_Init(self,
    ...
    import pickle
    with open('./Telescope/states/states', 'wb') as f:
  ...
  def type_define(self):
    """
    This function type define all the state variables
    to be used for ohter systems.
    =================================================
    """
    # Setting the state vectors - input, output and states
    self.state["y"] = self.state["y"].astype(self.size_c)
    self.state["x"] = self.state["x"].astype(self.size_c)
    self.state["u"] = self.state["u"].astype(self.size_c)
    # Setting the state space model matrices 
    self.state["A"] = self.state["A"].astype(self.size_c)
    self.state["B"] = self.state["B"].astype(self.size_c)
    self.state["C"] = self.state["C"].astype(self.size_c)
    self.state["D"] = self.state["D"].astype(self.size_c)
